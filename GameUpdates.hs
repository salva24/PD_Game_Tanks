...
updateGame dt state
    ...
    | otherwise = nuevo_state
        where 
            ...
            -- añadir explosiones por los robots que han muerto en este frame
            ...
            state7 = recursivaRobotsDecisiones (allRobots state5) state6 dt
...
------------------------------------------------------------------------------------------------------
{-- Funcion recursiva que para cada robot modifica el GameState preguntando y aplicando
...
-- funcion que incrementa el tiempo del GameState
incrementaTiempo :: Float -> GameState -> GameState
incrementaTiempo dt state = state { tiempo = tiempo state + dt }

-- borrar las explosione sque llevan mas de cierto tiempo vivas
...
-- funcion que para cada robot: actualiza su memoria
actualizaMemoriaRobots :: GameState -> GameState
actualizaMemoriaRobots state = state {allRobots = robots_actualizados}
    where
        robots_actualizados = map (actualizaMemoria state) (allRobots state)
...
-- función que pregunta al bot sus decisiones y las aplica al robot, comprobando si hay errores de posicion al girar el cuerpo.
...
preguntaEIntentaAplicarGiroYSiHayDisparo r0 state dt = (r_final,va_a_disparar)
    where
        ...
        r_final
            | compruebaErrorPosicionGiro state r3 = r2 -- si al girar el cuerpo no provoca colision, gira
            | otherwise                       = r3 -- si al girar el cuerpo provoca colision, no gira
                


-- Función que aplica el giro del cuerpo del robot   
...
-- Intneta aplicar el desplazamiento del robot y comprueba si provoca colisiones, si provoca colisones no lo aplica
...
intentaAplicarDesplazamiento r1 state dt = r_final
    where
        ...
        r_final
            | compruebaErrorPosicionDesplazamiento state r2 = r1 -- si al avanzar con su velocidad no provoca colision, avanza
            | otherwise                       = r2 -- si al avanzar causa colision, no avanza
...
--funcion que crea un nuevo proyectil y lo agnade a la lista del GameState
        
creaUnProyectilDisparado :: Int -> Robot -> Proyectil
creaUnProyectilDisparado nuevoId r = proyectil
    where
        direccion_proy = angleToVector (getAnguloDisparo r)
...
-- funcion que actualiza la posicion de proyectiles y borra los que se salen del mapa
...
-- funcion que calculas colisiones entre PROYECTILES y robots y mata aquellos que han sido impactados (borrando robot de la lista y poniendo un explosion)
...
processCollision state (RobotProjectileCollision r proyectil) =
    state { allRobots = updatedRobots, allProyectiles = updatedProyectiles }
  where
