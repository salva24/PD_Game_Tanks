...
projectPoint :: Point -> Vector Float -> Float
...
projectVertices :: [Point] -> Vector Float -> (Float, Float)
...
rectangleToVertices (Rectangle (Vector cx cy) (w, h) ang) = [(Vector (cx + x) (cy + y)) | (Vector x y) <- rotatedPoints]
    where halfW = w / 2
          halfH = h / 2
          -- Definimos los 4 vértices
          p1 = Vector (-halfW) (-halfH)  -- inferior izquierda
          p2 = Vector halfW (-halfH)  -- inferior derecha
          p3 = Vector halfW halfH    -- superior derecha
          p4 = Vector (-halfW) halfH     -- superior izquierda
...
checkCollision rect1 rect2 = all checkAxis allejes
    where vertices1 = rectangleToVertices rect1
          ...
          checkAxis eje = solapan proj1 proj2 -- Miramos si las dos tuplas se cortan, i.e. si las sombras de los dos rectángulos en el eje se solapan
                where proj1 = projectVertices vertices1 eje --Coordenada minima y máxima respecto al eje de forma que la sombra del rectángulo queda acotada en el eje
...
detectRobotProjectileCollisions robots proyectiles =
  pure RobotProjectileCollision <*> aliveRobots <*> validProjectiles
  where aliveRobots = filter isRobotAlive robots -- Un robot que siga en la simulación
        validProjectiles = 
            [ p | p <- proyectiles,
                all (\r -> id_entidad r /= getIdLanzador p) aliveRobots, -- Evitamos que cuando se dispara un misil, el robot que lo ha lanzado colisione con él
                any (\r -> checkCollision (robotToRectangle r) (projectileToRectangle p)) aliveRobots -- Comprobamos si han colisionado
            ]
--Equivalente a 
-- detectRobotProjectileCollisions robots proyectiles = 
...
detectRobotRobotCollisions robots = filter validCollision $ pure RobotRobotCollision <*> aliveRobots <*> aliveRobots
    where aliveRobots = filter isRobotAlive robots -- Un robot que siga en la simulación
          validCollision (RobotRobotCollision r1 r2) =
